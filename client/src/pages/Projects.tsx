    ...
    return project ? (
        <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
            <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
                <div className='flex item-center gap-2 sm:min-w-90 text-nowrap'>
                ...
                <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
                    ...
                    <Link target='_blank' to={`/preview/${project.id}`} className='flex items-center gap-2 px-4 py-1 hover:bg-gray-500 rounded sm:rounded-sm transition-colors border border-gray-700'>
            ...
            <div className='flex-1 flex overflow-auto'>
                <Sidebar project={project} setProject={(p) => setProject(p)} isGenerating={isGenerating} setIsGenerating={(v: boolean) => { setIsGenerating(v); if (v) setGenerationStartTime(Date.now()); }} isMenuOpen={isMenuOpen} />
                <div className='flex-1 p-2 pl-0'>
                    <ProjectPreview ref={previewRef} project={project} isGenerating={isGenerating} device={device} generationStartTime={generationStartTime} />
                </div>
            </div>
        </div>
    ) : (
